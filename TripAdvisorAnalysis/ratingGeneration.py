...
df = pd.read_csv('../datasets/ratings/sampled_reviews_left_skew.csv')
df = df.sample(n=1000)
samples = len(df)


enc = tiktoken.encoding_for_model("gpt-3.5-turbo")
possible_nums=range(1,6) # int from 1 to 5
...
async def generateRating(row):
    ...
    row_num = row.name
    prompt = f"The following is a review from a customer who stayed at a hotel. Please rate the hotel on a scale from 1 to 5, with 1 being the worst and 5 being the best. Number only. \n\nReview: {review} \n\nRating: "
    try:
...
if __name__ == "__main__":
    ...
    results_df.to_csv('ratings_comparison_left_skew.csv', index=False)
